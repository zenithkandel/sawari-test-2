...
class VehicleValidator {
    public static function validate(array $data, bool $isUpdate = false): array {
        ...
        if (!$isUpdate) {
            if (empty($data['name'])) $errors[] = 'Vehicle name is required';
        ...
        if (isset($data['speed'])) {
            if (floatval($data['speed']) < 0) $errors[] = 'Speed cannot be negative';
        }

        if (isset($data['bearing'])) {
            $b = intval($data['bearing']);
            if ($b < 0 || $b > 359) $errors[] = 'Bearing must be between 0 and 359';
        }

        if (isset($data['ratingAverage'])) {
            $r = floatval($data['ratingAverage']);
            if ($r < 0 || $r > 5) $errors[] = 'Rating average must be between 0 and 5';
        }

        if (isset($data['ratingCount'])) {
            if (intval($data['ratingCount']) < 0) $errors[] = 'Rating count cannot be negative';
        }
    ...
    public static function defaults(array $data): array {
        ...
        if ($data['ratingCount'] === 0) $data['ratingAverage'] = 0;
        return $data;
    }
}
